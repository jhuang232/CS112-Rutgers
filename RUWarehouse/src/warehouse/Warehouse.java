...
public class Warehouse {
    ...
    private void fixHeap(int id) {
        // IMPLEMENT THIS METHOD
        int n = sectors[id % 10].getSize();
        sectors[id % 10].swim(n);
    }
    ...
    private void evictIfNeeded(int id) {
        ...
        // not full done
        if (n == 5) {
            sectors[secNum].swap(1, n);
            sectors[secNum].deleteLast();
            sectors[secNum].sink(1);
        }
    }
    ...
    public void deleteProduct(int id) {
        ...
        int secNum = id % 10;
        for (int i = 1; i < sectors[id % 10].getSize() + 1; i++) {
            if (sectors[secNum].get(i).getId() == id) {
                sectors[secNum].swap(i, sectors[id % 10].getSize());
                sectors[secNum].deleteLast();
                sectors[secNum].sink(i);
            }
        }
    }
    ...
    public void purchaseProduct(int id, int day, int amount) {
        ...
        int secNum = id % 10;
        for (int i = 1; i < sectors[id % 10].getSize() + 1; i++) {
            if (sectors[secNum].get(i).getId() == id && sectors[secNum].get(i).getStock() >= amount) {
                sectors[secNum].get(i).setLastPurchaseDay(day);
                sectors[secNum].get(i).updateStock(amount * -1);
                sectors[secNum].get(i).updateDemand(amount);
                sectors[secNum].sink(i);
            }
        }
    }
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        ...
        int startSec = (id + 1) % 10;
        if (sectors[id % 10].getSize() == 5) {
            for (int i = startSec; i != (id % 10); i = (i + 1) % 10) {
                if (sectors[i].getSize() < 5) {
                    sectors[i].add(addProd);
